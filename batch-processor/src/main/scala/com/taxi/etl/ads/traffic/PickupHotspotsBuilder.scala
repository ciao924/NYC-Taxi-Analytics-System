...
object PickupHotspotsBuilder {

  private val logger = LoggerFactory.getLogger(getClass)
  private val TABLE_NAME = AdsConstants.TableNames.PICKUP_HOTSPOTS
    ...
    try {
      ...
      try {
        ...
        val filledDf = BaseAdsWriter.fillNullValues(
          resultDf,
          numericFields = Seq("trip_count", "total_revenue"),
        ...
        // 添加日志
        logger.info(s"  PickupHotspots 预期最小行数: $minExpectedRows (${days}天 × $minExpectedRowsMultiplier)")
