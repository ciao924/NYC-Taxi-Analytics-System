...
object FeeTrendBuilder {

  private val logger = LoggerFactory.getLogger(getClass)
  private val TABLE_NAME = AdsConstants.TableNames.FEE_TREND
    ...
    try {
      ...
      val resultDf = filteredDf.groupBy($"stat_date", $"taxi_type")
        .agg(
          ...
          round(avg($"cashless_rate"), 2).as("cashless_rate")
        )
        .withColumn("revenue_growth",
          coalesce(
            round(($"total_revenue" - lag($"total_revenue", 1).over(
              Window.partitionBy($"taxi_type").orderBy($"stat_date"))) /
              lag($"total_revenue", 1).over(
                Window.partitionBy($"taxi_type").orderBy($"stat_date")) * 100, 2),
            lit(0.0)
          ))
        .select(
          $"stat_date",
      ...
      val filledDf = BaseAdsWriter.fillNullValues(
        resultDf,
        numericFields = Seq("trip_count", "total_revenue", "avg_fare", "total_tip", "avg_tip_rate", "cashless_rate", "revenue_growth"),
