...
object FeePercentageBuilder {

  private val logger = LoggerFactory.getLogger(getClass)
  private val TABLE_NAME = AdsConstants.TableNames.FEE_PERCENTAGE
  ...
  def build(
    ...
    try {
      ...
      val totalDf = filteredDf.groupBy($"stat_date")
        .agg(sum($"total_amount").as("grand_total"))
      ...
      val resultDf = filteredDf.groupBy($"stat_date")
        ...
        .join(totalDf, Seq("stat_date"))
        .select(
          $"stat_date",
          explode(array(
            struct(lit("fare").as("fee_code"), lit("基础车费").as("fee_name"), round($"fare" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("extra").as("fee_code"), lit("附加费").as("fee_name"), round($"extra" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("mta_tax").as("fee_code"), lit("MTA税").as("fee_name"), round($"mta_tax" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("tip").as("fee_code"), lit("小费").as("fee_name"), round($"tip" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("tolls").as("fee_code"), lit("过路费").as("fee_name"), round($"tolls" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("improvement").as("fee_code"), lit("改善附加费").as("fee_name"), round($"improvement" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("congestion").as("fee_code"), lit("拥堵费").as("fee_name"), round($"congestion" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("airport").as("fee_code"), lit("机场费").as("fee_name"), round($"airport" / $"grand_total" * 100, 2).as("percentage")),
            struct(lit("cbd").as("fee_code"), lit("CBD拥堵费").as("fee_name"), round($"cbd" / $"grand_total" * 100, 2).as("percentage"))
          )).as("fee")
        )
        .select(
          $"stat_date",
      ...
      val filledDf = BaseAdsWriter.fillNullValues(
        resultDf,
        numericFields = AdsConstants.NumericFields.FEE_PERCENTAGE_FIELDS,
      ...
      val qualityPass = BaseAdsWriter.quickQualityCheck(
        ...
        requiredFields = Seq("stat_date", "fee_code", "percentage"),
